  ...
  <div class="md:grid md:grid-cols-3 md:gap-6">
    <div class="md:col-span-1">
      <div class="px-4 sm:px-0">
        ...
        <p class="mt-1 text-sm text-gray-600">Edit all the Product details visible to the customers.</p>
      </div>
    </div>
          ...
          <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
            <div class="grid grid-cols-6 gap-6">
              ...
              <div class="col-span-6 sm:col-span-3">
                ...
                <select required id="category_id" name="category_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                  @foreach ($categories as $category)
                    <option @if ($category->id == $product->sub_category->category->id) selected @endif value="{{ $category->id }}">{{ $category->name }}</option>
                  @endforeach
                </select>
              </div>
              ...
              <div class="col-span-6 sm:col-span-3">
                ...
                <select required id="sub_category_id" name="sub_category_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                  @foreach ($sub_categories as $sub_category)
                    <option @if ($sub_category->id == $product->sub_category->id) selected @endif name="{{ $sub_category->category->id }}" value="{{ $sub_category->id }}">{{ $sub_category->name }}</option>
                  @endforeach
                </select>
              </div>
              ...
              <div class="col-span-6 sm:col-span-4">
                ...
                <input type="text" required value="{{ $product->name }}" placeholder="Casual Rust T-Shirt" name="name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('name') border border-red-500 @enderror">
              </div>

              <div class="col-span-6">
                <label for="description" class="block text-sm font-medium text-gray-700">Product Description</label>
                <input type="text" required value="{{ $product->description }}" placeholder="Upgrade your wardrobe with our Casual Rust T-Shirt and enjoy the perfect combination of style and comfort." name="description" id="street-address" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('description') border border-red-500 @enderror">
              </div>

              <div class="col-span-6">
                <label for="details" class="block text-sm font-medium text-gray-700">Product Details</label>
                <div class="mt-1">
                  <textarea id="details" required name="details" rows="3" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md @error('details') border border-red-500 @enderror">{{ $product->details }}</textarea>
                </div>
              ...
              <div class="col-span-6 sm:col-span-6 lg:col-span-2">
                ...
                <input type="number" required value="{{ $product->unit_price }}" placeholder="100" name="unit_price" id="unit_price" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
              </div>

              <div class="col-span-6 sm:col-span-3 lg:col-span-2">
                <label for="quantity" class="block text-sm font-medium text-gray-700">Quantity</label>
                <input type="number" required value="{{ $product->quantity }}" placeholder="100" name="quantity" id="quantity" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
              </div>

              <div class="col-span-6 sm:col-span-3 lg:col-span-2">
                <label for="discount" class="block text-sm font-medium text-gray-700">Discount in %</label>
                <input type="number" required value="{{ $product->discount ?? 0 }}" placeholder="0" name="discount" id="discount" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
              </div>
              ...
              @foreach ($product->colors as $color)
                <div class="col-span-6 sm:col-span-2 flex items-end" name="{{ $color->name }}">
                  <input type="text" readonly value="{{ $color->name }}" name="color_names[]" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>

                <div class="col-span-6 sm:col-span-2" name="{{ $color->name }}">
                  <input type="text" readonly value="{{ $color->value }}" name="color_values[]" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>

                <div class="col-span-6 sm:col-span-2 flex items-center justify-end" name="{{ $color->name }}">
              ...
              @foreach ($product->sizes as $size)
                <div class="col-span-6 sm:col-span-2 flex items-end" name="{{ $size->name }}">
                  <input type="text" readonly value="{{ $size->name }}" name="sizes[]" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>

                <div class="col-span-6 sm:col-span-4 flex items-center justify-end" name="{{ $size->name }}">
                  <button type="button" class="text-indigo-600 hover:text-indigo-500" name="remove-btn">Remove</button>
                </div>
              @endforeach
            ...
            <div>
              ...
              <div id="catlog-image-container" class="mt-1 flex flex-col flex-wrap lg:flex-row items-center gap-4 mt-4">
                @foreach ($product->images as $image)
                  <input type="hidden" name="product_images[]" value="{{ $image->url }}" id="{{ $image->url }}">
                  <div class="relative" name="catlog-image">
                    ...
                    <img class="object-cover w-full lg:w-44" style="aspect-ratio: 1/1" src="{{ asset($image->url) }}" name="{{ $image->url }}">
                  </div>
                @endforeach
...
<script defer>
  ...
  const addProductColor = () => {
    ...
    colorNameElem.firstElementChild.setAttribute("name", "color_names[]");
    colorValueElem.firstElementChild.setAttribute("name", "color_values[]");
  ...
  const addProductSize = () => {
    ...
    sizeElem.firstElementChild.setAttribute("name", "sizes[]");
